!
!  serverhelper.f90 - This file contains helper functions for the COM server
!
...
module ServerHelper
  ...
  contains
    ...
    function RegisterServer () result (r)
        ...
        bDual = .FALSE.
        bDual = .TRUE.
        status = RegisterClass(UnicodeNotation_PROGID, UnicodeNotation_VERSION, &
                    UnicodeNotation_CLASS_NAME, UnicodeNotation_THREADING_MODEL, &
                    CLSID_UnicodeNotation, GUID_TypeLib_BOUN, modname, &
                    bDual)
    ...
    function UnregisterServer () result (r)
        ...
        r = S_OK
        !  Per class
        status = UnregisterClass(UnicodeNotation_PROGID, UnicodeNotation_VERSION, &
                    CLSID_UnicodeNotation)
        if (status /= S_OK) r = SELFREG_E_CLASS 

        !  UnRegister the type library
        !  UnRegisterTypeLib is not present on Win 95 unless DCOM 1.2
        !  is installed
        !status = UnRegisterTypeLib(GUID_TypeLib_BOUN, ...)
        !  Remove the \TYPELIB sub-keys
        call COMStringFromGUID(GUID_TypeLib_BOUN, TypelibGUID, status)
